...
| L4 | GPU `__constant__` 场景 | 常量内存广播+缓存，削减全局流量 | **反例**：实测反而更慢，见 PERFORMANCE.md |
...
- `results.md`：自动生成的 Markdown 报告（含加速阶梯说明与解读），**每次运行会覆盖**。
- `PERFORMANCE.md`：**人工整理的性能分析报告** —— 汇总各方案数据、逐层解释收益来源，并附一组球数扩展性实验（证明 BVH 的加速随球数增长：132 球 1.9x → 516 球 2.2x → 1028 球 3.3x）。
...
## 注意事项
- 常量内存上限 64KB，L4 最多约 1024 个球；**超出时会自动打印 WARN 并退回全局内存朴素路径**（数值结果与 L3 一致，不会中断）。
