...
REM --- 编译参数 ---
set NVCC_FLAGS=-arch=sm_86 -Iinclude -Xcompiler /utf-8 -Xcompiler /openmp
set SRCS=src\main.cu src\cpu_serial.cu src\cpu_omp.cu src\rt_naive.cu src\rt_constant.cu src\rt_opt.cu src\bvh_build.cu src\rt_bvh.cu src\report.cu

echo [INFO] 开始编译 (nvcc %NVCC_FLAGS%) ...
nvcc %NVCC_FLAGS% %SRCS% -o raytracer_demo.exe
if %errorlevel%==0 (
    echo.
    echo Build OK. 运行：raytracer_demo.exe  （可选参数见 --help）
) else (
    echo.
    echo Build FAILED.
    exit /b 1
